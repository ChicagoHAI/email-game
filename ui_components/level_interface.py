...
def show_level_progression_logic(level: float):
    ...
    if level in MULTI_TURN_LEVELS:
        ...
        st.info("🎯 **Level 2**: Choose your strategy wisely - some approaches may lead to additional challenges!")
    elif level == 2.5:
        st.warning("⚠️ **Challenge Level 2.5**: Forbidden strategies (layoffs, salary increases) are not allowed here!")
    elif level == 3:
        st.info("💼 **Level 3**: Navigate complex multi-stakeholder communication.")
    elif level == 5:
